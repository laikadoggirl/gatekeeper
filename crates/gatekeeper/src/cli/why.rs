...
pub fn run(json: bool) -> i32 {
    let path = match gatekeeper_core::policy::data_dir() {
        Some(d) => d.join("last_trigger.json"),
        None => {
            eprintln!("gatekeeper: cannot determine data directory");
            return 1;
        }
    };

    if !path.exists() {
        eprintln!("gatekeeper: no recent trigger found");
        return 1;
    }
    ...
    let content = match fs::read_to_string(&path) {
        Ok(c) => c,
        Err(e) => {
            eprintln!("gatekeeper: failed to read last trigger: {e}");
            return 1;
        }
    };
    ...
    let val: serde_json::Value = match serde_json::from_str(&content) {
        Ok(v) => v,
        Err(e) => {
            eprintln!("gatekeeper: failed to parse last trigger: {e}");
            return 1;
        }
    };

    eprintln!("gatekeeper: last trigger");
