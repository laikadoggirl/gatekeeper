...
pub fn run(shell: &str, json: bool) -> i32 {
    ...
    if let Err(e) = std::io::stdin().read_to_end(&mut raw_bytes) {
        eprintln!("gatekeeper: failed to read stdin: {e}");
        return 1;
    }
    ...
    if verdict.action != gatekeeper_core::verdict::Action::Allow {
        write_last_trigger(&verdict, &ctx.input);
    }

    // Log to audit
    let event_id = uuid::Uuid::new_v4().to_string();
    gatekeeper_core::audit::log_verdict(&verdict, &ctx.input, None, Some(event_id));
...
fn write_last_trigger(verdict: &gatekeeper_core::verdict::Verdict, cmd: &str) {
    if let Some(dir) = gatekeeper_core::policy::data_dir() {
        ...
        struct LastTrigger<'a> {
            ...
            findings: &'a [gatekeeper_core::verdict::Finding],
            timestamp: String,
        }
