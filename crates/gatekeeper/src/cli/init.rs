    ...
    match shell {
        "zsh" => {
            if let Some(dir) = &hook_dir {
                ...
                eprintln!("gatekeeper: could not locate or materialize shell hooks.");
                return 1;
            }
            0
        }
        "bash" => {
            if let Some(dir) = &hook_dir {
                println!(r#"export GATEKEEPER_BASH_MODE=enter"#);
        ...
        _ => {
            eprintln!("gatekeeper: unsupported shell '{shell}'");
...
/// 1. GATEKEEPER_SHELL_DIR env var (explicit override)
/// 2. ../share/gatekeeper/shell relative to binary (Homebrew layout)
/// 3. /usr/share/gatekeeper/shell (.deb layout)
...
pub fn find_hook_dir() -> Option<PathBuf> {
    ...
    if let Ok(dir) = std::env::var("GATEKEEPER_SHELL_DIR").or_else(|_| std::env::var("TIRITH_SHELL_DIR")) {
    ...
    if let Ok(exe) = std::env::current_exe() {
        if let Some(bin_dir) = exe.parent() {
            // 2. Homebrew layout: ../share/gatekeeper/shell
            let brew_dir = bin_dir.join("../share/gatekeeper/shell");
            if brew_dir.join("lib").exists() {
                return Some(brew_dir.canonicalize().unwrap_or(brew_dir));
            }

            // 3. System package layout: /usr/share/gatekeeper/shell
            #[cfg(unix)]
            {
                let sys_dir = PathBuf::from("/usr/share/gatekeeper/shell");
...
pub fn find_hook_dir_readonly() -> Option<PathBuf> {
    ...
    if let Some(data_dir) = gatekeeper_core::policy::data_dir() {
...
fn materialize_hooks() -> Option<PathBuf> {
    ...
    if needs_write {
        ...
        fs::write(shell_dir.join("gatekeeper.sh"), assets::TIRITH_SH).ok()?;
        ...
        eprintln!(
            "gatekeeper: materialized shell hooks to {}",
            shell_dir.display()
        );
    }
