...
pub fn run(json: bool) -> i32 {
    ...
    if json {
        match serde_json::to_string_pretty(&info) {
            Ok(s) => println!("{s}"),
            Err(e) => {
                eprintln!("gatekeeper: JSON serialization failed: {e}");
                return 1;
            }
        }
    } else {
...
fn gather_info() -> DoctorInfo {
    ...
    let hooks_materialized = hook_dir
        ...
        .map(|d| {
            ...
            if let Some(data) = gatekeeper_core::policy::data_dir() {
                d.starts_with(&data)
            } else {
                false
            }
    ...
    // Check if shell profile has gatekeeper init configured (legacy tirith detected for compatibility)
    let (shell_profile, hook_configured) = check_shell_profile(&detected_shell);

    let data_dir = gatekeeper_core::policy::data_dir();
    ...
    // User-level policy
    if let Some(config) = gatekeeper_core::policy::config_dir() {
    ...
    // GATEKEEPER_POLICY_ROOT override (legacy TIRITH_POLICY_ROOT supported)
    let policy_root_env = std::env::var("GATEKEEPER_POLICY_ROOT").ok().or_else(|| std::env::var("TIRITH_POLICY_ROOT").ok());
    if let Some(ref root) = policy_root_env {
        let tirith_dir = PathBuf::from(root).join(".gatekeeper");
        for ext in &["policy.yaml", "policy.yml"] {
...
fn print_human(info: &DoctorInfo) {
    eprintln!("gatekeeper {}", info.version);
    ...
    if info.hook_configured {
        ...
        eprintln!();
        eprintln!("  WARNING: gatekeeper shell hook is not configured!");
        eprintln!("  Commands will NOT be intercepted until you add to your shell profile:");
        eprintln!();
        match info.detected_shell.as_str() {
            "zsh" => {
                eprintln!("    echo 'eval \"$(gatekeeper init)\"' >> ~/.zshrc");
                eprintln!("    source ~/.zshrc");
            }
            "bash" => {
                eprintln!("    echo 'eval \"$(gatekeeper init)\"' >> ~/.bashrc");
                eprintln!("    source ~/.bashrc");
            }
            "fish" => {
                eprintln!("    echo 'gatekeeper init | source' >> ~/.config/fish/config.fish");
                eprintln!("    source ~/.config/fish/config.fish");
            }
            _ => {
                eprintln!("    eval \"$(gatekeeper init)\"");
            }
        }
        eprintln!();
    }
    ...
    if let Some(ref root) = info.policy_root_env {
        eprintln!("  policy root:  {root} (GATEKEEPER_POLICY_ROOT/TIRITH_POLICY_ROOT)");
    }
    eprintln!(
        "  data dir:     {}",
...
/// Check if the user's shell profile contains gatekeeper init configuration (legacy tirith supported).
...
fn check_shell_profile(shell: &str) -> (Option<PathBuf>, bool) {
    ...
    // Find the first existing profile and check if it contains gatekeeper/tirith init
    for profile in &profile_candidates {
        if profile.exists() {
            if let Ok(contents) = std::fs::read_to_string(profile) {
                // Check for various forms of gatekeeper (or legacy tirith) init
                let configured = contents.contains("gatekeeper init")
                    || contents.contains("gatekeeper-hook")
                    || contents.contains("_gatekeeper_")
                    || contents.contains("tirith init")
