use gatekeeper_core::receipt::Receipt;
    ...
    match Receipt::list() {
        Ok(receipts) => {
            if let Some(r) = receipts.first() {
                ...
                eprintln!("gatekeeper: no receipts found");
                1
            }
        }
        Err(e) => {
        ...
        Ok(r) => match r.verify() {
            Ok(valid) => {
                if json {
                    ...
                    eprintln!("gatekeeper: receipt {} verified OK", &sha256[..12]);
                } else {
                    eprintln!("gatekeeper: receipt {} FAILED verification", &sha256[..12]);
                }
                if valid {
                    0
                } else {
            ...
            Err(e) => {
                eprintln!("gatekeeper: verify failed: {e}");
                1
            }
        },
        Err(e) => {
