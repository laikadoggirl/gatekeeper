...
/// These are written to the user data dir on first `gatekeeper init`.
///
/// Assets live under `crates/gatekeeper/assets/` so they are included in the
/// crate tarball and `cargo install` / `cargo publish` work correctly.
pub const TIRITH_SH: &str = include_str!(concat!(
    env!("CARGO_MANIFEST_DIR"),
    "/assets/shell/gatekeeper.sh"
));
