...
[[ -n "$_GATEKEEPER_ZSH_LOADED" ]] && return
_GATEKEEPER_ZSH_LOADED=1

# Save original accept-line widget if it exists
if zle -la | grep -q '^accept-line$'; then
  zle -A accept-line _gatekeeper_original_accept_line
fi

_gatekeeper_accept_line() {
  local buf="$BUFFER"

  # Empty input: pass through
  if [[ -z "$buf" ]]; then
    zle _gatekeeper_original_accept_line 2>/dev/null || zle .accept-line
    return
  fi

  # Run gatekeeper check. Binary prints warnings/blocks directly to stderr.
  gatekeeper check --shell posix -- "$buf"
  local rc=$?
...
zle -N accept-line _gatekeeper_accept_line

# Bracketed paste interception
if zle -la | grep -q '^bracketed-paste$'; then
  zle -A bracketed-paste _gatekeeper_original_bracketed_paste
fi

_gatekeeper_bracketed_paste() {
  # Read the pasted content into CUTBUFFER via the original widget
  local old_buffer="$BUFFER"
  local old_cursor="$CURSOR"
  zle _gatekeeper_original_bracketed_paste 2>/dev/null || zle .bracketed-paste
  ...
  if [[ -n "$pasted" ]]; then
    # Pipe pasted content to gatekeeper paste
    echo -n "$pasted" | gatekeeper paste --shell posix
    local rc=$?
...
zle -N bracketed-paste _gatekeeper_bracketed_paste
