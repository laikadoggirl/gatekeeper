...
pub fn run(opts: RunOptions) -> Result<RunResult, String> {
    ...
    if !opts.no_exec && !opts.interactive {
        return Err("gatekeeper run requires an interactive terminal or --no-exec flag".to_string());
    }
    ...
    eprintln!(
        "gatekeeper: downloaded {} bytes (SHA256: {})",
        content.len(),
        &sha256[..12]
    );
    eprintln!("gatekeeper: interpreter: {interpreter}");
    if analysis.has_sudo {
        eprintln!("gatekeeper: WARNING: script uses sudo");
    }
    if analysis.has_eval {
        eprintln!("gatekeeper: WARNING: script uses eval");
    }
    if analysis.has_base64 {
        eprintln!("gatekeeper: WARNING: script uses base64");
    }
    ...
    if !response_line.trim().eq_ignore_ascii_case("y") {
        eprintln!("gatekeeper: execution cancelled");
        receipt.save().map_err(|e| format!("save receipt: {e}"))?;
        return Ok(RunResult {
            receipt,
            executed: false,
