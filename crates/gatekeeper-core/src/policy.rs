...
impl Policy {
    ...
    pub fn discover(cwd: Option<&str>) -> Self {
        // Check env override first (new then legacy)
        if let Ok(root) = std::env::var("GATEKEEPER_POLICY_ROOT").or_else(|_| std::env::var("TIRITH_POLICY_ROOT")) {
            if let Some(path) = find_policy_in_dir(&PathBuf::from(&root).join(".gatekeeper").as_path())
                .or_else(|| find_policy_in_dir(&PathBuf::from(&root).join(".tirith")))
            {
    ...
    fn load_from_path(path: &Path) -> Self {
        match std::fs::read_to_string(path) {
            Ok(content) => match serde_yaml::from_str::<Policy>(&content) {
                ...
                Err(_) => {
                    eprintln!(
                        "gatekeeper: warning: failed to parse policy at {}",
                        path.display()
                    );
                    Policy::default()
                }
    ...
    /// Load and merge org-level lists from a repo root's .gatekeeper/ dir (legacy .tirith supported).
    pub fn load_org_lists(&mut self, cwd: Option<&str>) {
        if let Some(repo_root) = find_repo_root(cwd) {
            let org_dir = if repo_root.join(".gatekeeper").exists() { repo_root.join(".gatekeeper") } else { repo_root.join(".tirith") };
...
fn discover_policy_path(cwd: Option<&str>) -> Option<PathBuf> {
    ...
    loop {
        // Check for .gatekeeper/ or legacy .tirith/
        if let Some(candidate) = find_policy_in_dir(&current.join(".gatekeeper")).or_else(|| find_policy_in_dir(&current.join(".tirith"))) {
...
pub fn config_dir() -> Option<PathBuf> {
    let base = etcetera::choose_base_strategy().ok()?;
    let gate = base.config_dir().join("gatekeeper");
    if gate.exists() { Some(gate) } else { Some(base.config_dir().join("tirith")) }
}
