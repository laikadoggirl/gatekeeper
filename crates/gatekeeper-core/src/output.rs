...
pub fn write_human(verdict: &Verdict, mut w: impl Write) -> std::io::Result<()> {
    ...
    writeln!(w)?;
    writeln!(w, "gatekeeper: {action_str}")?;
...
/// If stderr is not a TTY, strip ANSI colors. When pretty is enabled and on a
/// TTY, render styled output using anstream/anstyle.
pub fn write_human_auto(verdict: &Verdict) -> std::io::Result<()> {
    let stderr = std::io::stderr();
    let is_tty = is_terminal::is_terminal(&stderr);

    if is_tty && is_pretty_enabled() {
        return write_human_pretty(verdict);
    }
...
fn is_pretty_enabled() -> bool {
    match std::env::var("TIRITH_PRETTY") {
        Ok(v) if v == "0" => false,
        Ok(v) if v == "1" => true,
        _ => true,
    }
...
fn write_human_pretty(verdict: &Verdict) -> std::io::Result<()> {
    if verdict.findings.is_empty() {
        return Ok(());
    }

    let stderr = std::io::stderr();
    let mut out = AutoStream::auto(stderr.lock());

    let (action_label, action_style) = match verdict.action {
        Action::Allow => ("ALLOW", Style::new().fg_color(Some(Color::Ansi(AnsiColor::Green)))) ,
        Action::Warn => ("WARNING", Style::new().fg_color(Some(Color::Ansi(AnsiColor::Yellow))).effects(Effects::BOLD)) ,
        Action::Block => ("BLOCKED", Style::new().fg_color(Some(Color::Ansi(AnsiColor::Red))).effects(Effects::BOLD)) ,
    };

    let reset = Style::new();
    let findings_count = verdict.findings.len();
    writeln!(out)?;
    writeln!(
        out,
        "{action_style}gatekeeper:{reset} {action_style}{action_label}{reset} ({} finding{})",
        findings_count,
    ...
    for (i, finding) in verdict.findings.iter().enumerate() {
        let sev_style = match finding.severity {
            Severity::Critical => Style::new().fg_color(Some(Color::Ansi(AnsiColor::BrightRed))).effects(Effects::BOLD),
            Severity::High => Style::new().fg_color(Some(Color::Ansi(AnsiColor::Red))).effects(Effects::BOLD),
            Severity::Medium => Style::new().fg_color(Some(Color::Ansi(AnsiColor::Yellow))),
            Severity::Low => Style::new().fg_color(Some(Color::Ansi(AnsiColor::Cyan))),
        };
        let title_style = Style::new().effects(Effects::BOLD);
        writeln!(
            out,
            "  {sev_style}{idx}) [{severity}]{reset} {title_style}{title}{reset} {dim}({rule}){reset}",
            idx = i + 1,
    ...
    // Details only for findings with homoglyph evidence
        ...
        for evidence in &finding.evidence {
            ...
            } = evidence
            {
                if !any_details {
                    ...
                    writeln!(out, "{label}Details:{reset}")?;
                }
                let label = Style::new().effects(Effects::BOLD);
                let esc = Style::new().fg_color(Some(Color::Ansi(AnsiColor::Yellow)));
                let visual = format_visual_with_markers(raw, suspicious_chars);
                writeln!(out, "    {label}Visual:{reset}  {visual}")?;
                writeln!(out, "    {label}Escaped:{reset} {esc}{escaped}{reset}")?;
                if !suspicious_chars.is_empty() {
                    writeln!(out, "    {label}Suspicious bytes:{reset}")?;
                    for sc in suspicious_chars {
                        writeln!(out, "      {:08x}: {} {:6} {}", sc.offset, sc.hex_bytes, sc.codepoint, sc.description)?;
                    }
                }
            }
        }
