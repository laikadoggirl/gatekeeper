...
use gatekeeper_core::verdict::Action;
    ...
    for filename in &files {
        ...
        for fixture in &fixtures {
            ...
            if scan_context == ScanContext::Paste {
                ...
                let byte_scan = gatekeeper_core::extract::scan_bytes(bytes);
            ...
            if scan_context == ScanContext::Exec {
                let byte_scan = gatekeeper_core::extract::scan_bytes(fixture.input.as_bytes());
                if byte_scan.has_bidi_controls || byte_scan.has_zero_width {
                    continue;
                }
            }

            let regex_triggered = gatekeeper_core::extract::tier1_scan(&fixture.input, scan_context);
...
fn test_rule_id_list_is_complete() {
    use gatekeeper_core::verdict::RuleId;
...
fn test_extractor_ids_cover_rule_triggers() {
    let ids: HashSet<&str> = gatekeeper_core::extract::extractor_ids()
        .iter()
        .copied()
        .collect();
