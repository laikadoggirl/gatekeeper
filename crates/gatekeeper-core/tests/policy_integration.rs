...
use gatekeeper_core::engine::{self, AnalysisContext};
use gatekeeper_core::extract::ScanContext;
use gatekeeper_core::tokenize::ShellType;
use gatekeeper_core::verdict::{Action, RuleId, Severity};

/// Create a temp dir that looks like a repo root with a `.gatekeeper/` policy.
fn make_repo(policy_yaml: &str) -> TempDir {
    let tmp = TempDir::new().expect("create temp dir");
    fs::create_dir_all(tmp.path().join(".git")).unwrap();
    let tirith_dir = tmp.path().join(".gatekeeper");
    fs::create_dir_all(&tirith_dir).unwrap();
    fs::write(tirith_dir.join("policy.yaml"), policy_yaml).unwrap();
    tmp
}

fn analyze_exec(input: &str, cwd: &str) -> gatekeeper_core::verdict::Verdict {
...
fn test_blocklist_triggers_policy_blocklisted() {
    ...
    fs::write(
        repo.path().join(".gatekeeper/blocklist"),
...
fn test_blocklist_case_insensitive() {
    let repo = make_repo("fail_mode: open\n");
    fs::write(repo.path().join(".gatekeeper/blocklist"), "MALICIOUS.COM\n").unwrap();
...
fn test_blocklist_substring_match() {
    let repo = make_repo("fail_mode: open\n");
    fs::write(repo.path().join(".gatekeeper/blocklist"), "evil.com\n").unwrap();
...
fn test_allowlist_filters_findings() {
    let repo = make_repo("fail_mode: open\n");
    fs::write(repo.path().join(".gatekeeper/allowlist"), "bit.ly\n").unwrap();
...
fn test_org_lists_merged_into_policy() {
    let repo = make_repo("fail_mode: open\n");
    let tirith_dir = repo.path().join(".gatekeeper");
