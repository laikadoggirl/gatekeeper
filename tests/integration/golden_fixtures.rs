...
fn run_fixture(fixture: &Fixture) {
    ...
    let expected_action = match fixture.expected_action.as_str() {
        ...
        "warn" => Action::Warn,
        "block" => Action::Block,
        other => panic!("Unknown expected_action: {} in fixture {}", other, fixture.name),
    };
...
fn test_fixture_count() {
    ...
    assert!(
        total >= 200,
        "Expected at least 200 golden fixtures, found {}",
        total
    );
}
...
fn test_tier1_coverage() {
    ...
    for filename in &files {
        ...
        for fixture in &fixtures {
            ...
            let regex_triggered =
                tirith_core::extract::tier1_scan(&fixture.input, scan_context);
